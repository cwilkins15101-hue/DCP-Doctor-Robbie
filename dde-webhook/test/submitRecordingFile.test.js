...
const { handler } = require('../src/functions/submitRecordingFile');
...
// Deliberately no Authorization/Entra-token requirement here, unlike
// submitRecording.js (AAS WebSocket) -- this REST path uses dde-webhook's
// own app-only credentials (see audioUpload.js/dragonApiAuth.js), no
// physician sign-in involved.
test('succeeds with just the app secret, no Authorization header required', async () => {
...
test('orchestrates create session -> REST upload -> end session, and returns the correlationId', async () => {
  const res = await handler(
    fakeFormDataRequest({
      headers: { 'x-app-secret': 'test-app-secret' },
      fields: {

...
// True live streaming, not store-and-forward: streamRecording() above waits
// for a complete, already-finished recording, then dumps the whole thing
// through the wire in a tight loop -- there is no "live" in it at all, and
// that mismatch (a real-time protocol fed an unrealistic instantaneous
// burst) is the leading theory (2026-09-24) for why every fully
// protocol-correct streamRecording() attempt gets total silence rather than
// any response. This instead opens the connection immediately and forwards
// each chunk of `incomingChunks` (an async-iterable of Buffers/Uint8Arrays —
// the web app's own live microphone capture, arriving in real time, not a
// pre-recorded file sliced up afterward) the moment it arrives.
//
// Genuine raw PCM (not a compressed/containerized format like WebM) is what
// makes this safe to chunk arbitrarily in the first place -- any byte range
// of a raw sample stream is independently valid, unlike webmOpus (tried and
// reverted 2026-09-24: a real container format's random byte slices aren't
// independently valid fragments). dataFormat.pcm is hardcoded to Microsoft's
// documented recommendation (16-bit LE, 16000 Hz, mono) -- the web capture
// code (App.js) must actually produce audio matching this exactly, since
// this is a declaration, not a conversion.
async function streamRecordingLive({
  correlationId,
  incomingChunks,
  recordingId = 1,
  ehrInstanceId,
  ...
  if (!entraUserToken) {
    throw new Error('streamRecordingLive requires entraUserToken (the AAS WebSocket rejects/ignores an app-only token).');
  }
  const customerId = config.dragonEnvironmentId();
  const wsRecordingId = crypto.randomUUID();
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', async () => {
      clearTimeout(openTimer);
      log(`[audioStreamUpload] (live) WebSocket open (correlationId=${correlationId}, recordingId=${wsRecordingId})`);
      try {
        const recordingOpenBody = {
          recordingId: wsRecordingId,
        ...
        ws.send(buildTextMessage('RecordingOpen', recordingOpenBody));
        log('[audioStreamUpload] (live) Sent RecordingOpen (dataFormat=pcm 16-bit/16kHz/mono), streaming live chunks as they arrive...');

        startedStreamingAt = Date.now();
        let offset = 0;
        let chunkCount = 0;
        for await (const chunk of incomingChunks) {
          ...
          ws.send(buildDataChunkFrame(offset, buf), { binary: true });
          offset += buf.length;
          chunkCount += 1;
        }
        const recordingLengthSeconds = Math.max(1, Math.round((Date.now() - startedStreamingAt) / 1000));
        log(`[audioStreamUpload] (live) Incoming stream ended (${chunkCount} chunks, ${offset} bytes total, ~${recordingLengthSeconds}s elapsed)`);
        ...
        log(`[audioStreamUpload] (live) Sent RecordingClose (recordingLengthSeconds=${recordingLengthSeconds}), waiting for RecordingCloseResponse...`);
    ...
    ws.on('message', (data, isBinary) => {
      if (isBinary) return;
      const raw = data.toString('utf8');
      log(`[audioStreamUpload] (live) Received text message: ${raw.slice(0, 500)}`);
      if (settled) return;
      const parsed = parseTextMessage(raw);
      if (parsed?.recordingCloses) {
        succeed({ dataStored: parsed.recordingCloses.dataStored });
    ...
    ws.on('close', (code, reasonBuf) => {
      log(`[audioStreamUpload] (live) WebSocket close event: code=${code}, reason=${reasonBuf?.toString('utf8') || '(none)'}, alreadySettled=${settled}`);
      if (settled) return;
      if (code === 1000) {
        succeed({ dataStored: null });
      } else {

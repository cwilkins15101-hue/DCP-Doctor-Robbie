...
//
// Restored 2026-09-24 for the manual "Upload a file" flow only (see
// submitRecordingFile.js) — replaced by the AAS WebSocket (see
// audioStreamUpload.js) for mic recordings on 2026-09-22, but that switch
// requires a delegated (real signed-in physician) Entra token, and today's
// live testing showed the WebSocket path stalling/timing out in ways this
// REST path historically didn't. Kept as a genuinely separate transport
// (a plain app-only, server-to-server call, no physician sign-in involved)
// rather than reused for anything else, since it doesn't support
// Voice-to-Form's outputFormIds field the way the WebSocket API does.
